...
from ..utils import capture_init, center_trim, unfold
...
class DConv(nn.Module):
    ...
    def __init__(self, channels: int, compress: float = 4, depth: int = 2, init: float = 1e-4,
        ...
        def norm_fn(d): return nn.Identity()  # noqa
        if norm:
            def norm_fn(d): return nn.GroupNorm(1, d)  # noqa
        ...
        for d in range(self.depth):
            ...
            mods = [
                nn.Conv1d(channels, hidden, kernel,
            ...
            if lstm:
                mods.insert(3, BLSTM(hidden, layers=2,
                                     max_steps=200, skip=True))
...
class LocalState(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.nfreqs:
            ...
            freq_kernel = torch.cos(
                2 * math.pi * delta / periods.view(-1, 1, 1))
            freq_q = self.query_freqs(x).view(
                B, heads, -1, T) / self.nfreqs ** 0.5
            dots += torch.einsum("fts,bhfs->bhts", freq_kernel, freq_q)
        if self.ndecay:
            decays = torch.arange(1, self.ndecay + 1,
                                  device=x.device, dtype=x.dtype)
            decay_q = self.query_decay(x).view(B, heads, -1, T)
            decay_q = torch.sigmoid(decay_q) / 2
            decay_kernel = - decays.view(-1, 1, 1) * \
                delta.abs() / self.ndecay**0.5
            dots += torch.einsum("fts,bhfs->bhts", decay_kernel, decay_q)

        # Kill self reference.
        dots.masked_fill_(
            torch.eye(T, device=dots.device, dtype=torch.bool), -100)
    ...
    def __init__(self,
        ...
        for index in range(depth):
            ...
            if index >= norm_starts:
                def norm_fn(d): return nn.GroupNorm(norm_groups, d)  # noqa
            ...
            if rewrite:
                decode += [
                    nn.Conv1d(channels, ch_scale * channels,
                              2 * context + 1, padding=context),
                    norm_fn(ch_scale * channels), activation]
            if dconv_mode & 2:
