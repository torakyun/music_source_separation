...
I would like to thank the following people for their significant contributions to music source separation. By introducing the ``Trainer`` class, I have made significant changes to the framework that repeats training, validation, and evaluation. However, the implementation files for the individual data processing and source separation models remain largely unchanged. In addition, in incorporating optimizations in the field of speech synthesis, I have used I also referred to the implementation by [kan-hayashi](https://github.com/kan-bayashi/ParallelWaveGAN).
